   ...
   steps {
    ...
    stage('Building web image') {
            steps{
                dir('app/web')
                {
                   sh "sudo docker build --build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` --build-arg VCS_REF=`git rev-parse --short HEAD` --build-arg IMAGE_TAG_REF=v1 -t ${env.WEB_IMAGE}:${env.BUILD_NUMBER} ."
                }
            }
       }
